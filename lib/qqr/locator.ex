defmodule QQR.Locator do
  ...
  defp group_finder_patterns(scored) do
    ...
    |> Enum.flat_map(fn {point, i} ->
      if i > @max_finder_patterns_to_search do
        []
      else
        others =
          scored
  ...
  defp bresenham_walk(x, y, to_x, dx, dy, error, x_step, y_step, to_y, steep, current_pixel,
         switch_points, max_switches, matrix) do
    if x == to_x + x_step do
      build_distances(switch_points, max_switches)
    else
      {real_x, real_y} = if steep, do: {y, x}, else: {x, y}
      pixel = BitMatrix.get(matrix, real_x, real_y)

      {switch_points, current_pixel} =
        if pixel != current_pixel do
          new_points = switch_points ++ [{real_x, real_y}]

          if Kernel.length(new_points) == max_switches + 1 do
            throw({:done, build_distances(new_points, max_switches)})
          end
      ...
      {new_y, new_error} =
        if new_error > 0 do
          if y == to_y do
            throw({:done, build_distances(switch_points, max_switches)})
          end

          {y + y_step, new_error - dx}
        else
          {y, new_error}
        end
  ...
  defp find_alignment_pattern(matrix, alignment_quads, top_right, top_left, bottom_left) do
    case compute_dimension(top_left, top_right, bottom_left, matrix) do
      {:ok, dimension, module_size} ->
        ...
        bottom_right = {tr_x - tl_x + bl_x, tr_y - tl_y + bl_y}
        ...
        expected = {
          tl_x + correction * (br_x - tl_x),
          tl_y + correction * (br_y - tl_y)
        }
        ...
        alignment_candidates =
          ...
          |> Enum.map(fn q ->
            ...
            if BitMatrix.get(matrix, floor(x), floor(y)) do
              size_score =
                score_pattern({floor(x), floor(y)}, @alignment_ratios, matrix)
        ...
        alignment =
          if modules_between >= 15 and alignment_candidates != [] do
            best = hd(alignment_candidates)
            {best.x, best.y}
          else
            expected
          end
